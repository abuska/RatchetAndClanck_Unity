...
public class GunInventory : MonoBehaviour
{
    ...
    IEnumerator UpdateIconsFromResources()
    {
        ...
        for (int i = 0; i < gunsIHave.Count; i++)
        {
            icons[i] =
                (Texture)
                Resources
                    .Load("Weap_Icons/" + gunsIHave[i].ToString() + "_img");
        }
    }
    ...
    IEnumerator Spawn(int _redniBroj)
    {
        ...
        if (currentGun)
        {
            if (currentGun.name.Contains("Gun"))
            {
                ...
                GameObject resource =
                    (GameObject)
                    Resources.Load(gunsIHave[_redniBroj].ToString());
                currentGun =
                    (GameObject)
        ...
        {
            GameObject resource =
                (GameObject) Resources.Load(gunsIHave[_redniBroj].ToString());
            currentGun =
                (GameObject)
                Instantiate(resource,
                transform.position, /*gameObject.transform.rotation*/
	...
	*/
    void AssignHandsAnimator(GameObject _currentGun)
    {
        ...
        {
            /*currentHAndsAnimator =
                currentGun.GetComponent<GunScript>().handsAnimator;*/
        }
    }
    ...
    {
        string deleteCloneFromName =
            currentGun.name.Substring(0, currentGun.name.Length - 7);
        ...
        if (menuStyle == MenuStyle.horizontal)
        {
            ...
            {
                GUI
                    .DrawTexture(new Rect(vec2(beginPosition).x +
                        (_number * position_x(spacing)),
            ...
            else
            {
                GUI
                    .DrawTexture(new Rect(vec2(beginPosition).x +
                        (_number * position_x(spacing) + 10),
                        vec2(beginPosition).y + 10, //position variables
                        vec2(size).x - 20,
                        vec2(size).y - 20), //size
                    icons[_number]);
            }
        ...
        else if (menuStyle == MenuStyle.vertical)
        {
            if (deleteCloneFromName == gunsIHave[_number])
            {
                GUI
                    .DrawTexture(new Rect(vec2(beginPosition).x,
                        vec2(beginPosition).y + (_number * position_y(spacing)), //position variables
                        vec2(size).x,
                        vec2(size).y), //size
                    icons[_number]);
            }
            else
            {
                GUI
                    .DrawTexture(new Rect(vec2(beginPosition).x,
                        vec2(beginPosition).y +
                        10 +
                        (_number * position_y(spacing)), //position variables
                        vec2(size).x - 20,
                        vec2(size).y - 20), //size
                    icons[_number]);
            }
    ...
    {
        return new Vector2(Screen.width * _vec2.x / 100,
            Screen.height * _vec2.y / 100);
    }
